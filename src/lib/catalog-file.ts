...
export const catalogRowSchema = z.object({
  ...
  /**
   * True when the SKU was generated by us rather than printed by the supplier.
   * Amul's catalogue carries no codes, so its SKUs are derived from name + pack
   * size. Deterministic, so re-imports still match — but if the supplier later
   * issues real codes, these need a one-time mapping.
   */
  skuGenerated: z.boolean().optional(),
  ...
  /**
   * Optional at the file level so a range can be captured before prices are
   * sourced — the Amul catalogue is a product poster with no prices at all.
   *
   * `Product.price` in the database is NOT nullable, so import-catalog holds
   * priceless rows back and names them rather than inventing a figure. Once a
   * price is filled in, the next import picks the row up.
   */
